  ...
  "UserController" should {
    ...
    "send 404 on a wrong User, Network or network_campaign_id request" in {
      TestDB_0.creating_and_filling_inMemoryDB() {
        val Some(res) = route(FakeRequest(GET, "/user/foo").withHeaders(("password" -> "123")))
    ...
    "send 404 or 400 on a wrong Password request" in {
      TestDB_0.creating_and_filling_inMemoryDB() {
        val Some(res0) = route(FakeRequest(GET, "/user/Coda").withHeaders(("password" -> "12345678")))
        status(res0) must equalTo(404) //NotFound

        val Some(res1) = route(FakeRequest(GET, "/user/Coda"))
    ...
    "respond User in json" in {
      TestDB_0.creating_and_filling_inMemoryDB() {
        val res0 = route(FakeRequest(GET, "/user/Coda").withHeaders(("password" -> "123"))).get
        
        status(res0) must equalTo(OK)
        contentType(res0) must beSome.which(_ == "application/json")
        contentAsString(res0) must contain("Coda")
      }
    }
